#include <cstdint>
#include <cstring>
#include <wiringPi.h> // Ajouté pour la fonction wiringPiSetupGpio
...
// Macros pour créer les données binaires des polices
...
// Nouvelles lettres ajoutées pour le message
const uint8_t D_data[] = FONT3(R3(1,1,0),R3(1,0,1),R3(1,0,1),R3(1,0,1),R3(1,1,0)); // D
const uint8_t S_data[] = FONT3(R3(1,1,1),R3(1,0,0),R3(1,1,1),R3(0,0,1),R3(1,1,1)); // S
const uint8_t I_data[] = FONT3(R3(1,1,1),R3(0,1,0),R3(0,1,0),R3(0,1,0),R3(1,1,1)); // I
const uint8_t B_data[] = FONT3(R3(1,1,0),R3(1,0,1),R3(1,1,0),R3(1,0,1),R3(1,1,0)); // B
const uint8_t E_data[] = FONT3(R3(1,1,1),R3(1,0,0),R3(1,1,1),R3(1,0,0),R3(1,1,1)); // E
const uint8_t Y_data[] = FONT3(R3(1,0,1),R3(1,0,1),R3(0,1,0),R3(0,1,0),R3(0,1,0)); // Y
const uint8_t U_data[] = FONT3(R3(1,0,1),R3(1,0,1),R3(1,0,1),R3(1,0,1),R3(1,1,1)); // U
const uint8_t J_data[] = FONT3(R3(0,1,1),R3(0,0,1),R3(0,0,1),R3(1,0,1),R3(0,1,0)); // J
...
const uint8_t COMMA_data[] = FONT3(R3(0,0,0),R3(0,0,0),R3(0,0,0),R3(0,1,0),R3(1,0,0)); // Virgule
...
// --- MESSAGE ET LOGIQUE DE SCROLL ---

const char* message = "DANS IMPOSSIBLE, IL Y A TOUJOURS UN POSSIBLE";
...
void scrollMessage(){
    ...
    for(int i=0; message[i]; i++){
        ...
        if(!fc) continue; // Ignore les caractères inconnus (sécurité)
...
int main() {
    // Initialisation WiringPi (ajout du include en haut nécessaire)
    ...
    while (true) {
        scrollMessage();
        delay(200); // Vitesse de défilement
    }
